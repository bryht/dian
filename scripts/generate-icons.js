...
async function generateIcons() {
    ...
    const faviconPng = fs.readFileSync(path.join(rootDir, 'public/favicon.png'));
    const faviconIco = await pngToIco(faviconPng);
    fs.writeFileSync(path.join(rootDir, 'public/favicon.ico'), faviconIco);
    fs.unlinkSync(path.join(rootDir, 'public/favicon.png')); // Clean up temp png
    ...
    fs.writeFileSync(path.join(rootDir, 'assets/icon.ico'), icoBuffer);
    console.log('✓ Generated assets/icon.ico');
    
    fs.writeFileSync(path.join(rootDir, 'assets/installerIcon.ico'), icoBuffer);
    ...
    await sharp(svgBuffer)
        .resize(512, 512)
        .png()
        .toFile(path.join(rootDir, 'build/icon.png'));
